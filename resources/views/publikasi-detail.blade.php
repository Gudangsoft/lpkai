...
<style>
    ...
    .pd-download-btn {
        ...
        transition: all 0.2s; box-shadow: 0 4px 14px rgba(26,111,196,0.3);
    }
    ...
    .pd-info-list li {
        ...
        font-size: 0.85rem; padding-bottom: 10px; border-bottom: 1px solid #f1f5f9;
    }
    ...
    .pd-related-title {
        font-size: 1.25rem; font-weight: 800; color: #0d2b5e;
    ...
    @media (max-width: 900px) {
        ...
        .pd-cover-mobile { display: block; }
        .pd-sidebar-card:first-child { display: none; }
...
<div class="pd-container">
    <div class="pd-main">
        @if($publikasi->gambar)
        <div class="pd-cover-mobile">
            <img src="{{ Storage::url($publikasi->gambar) }}" alt="{{ $publikasi->judul }}"
                style="width:100%;display:block;{{ $publikasi->kategori === 'Buku' ? 'max-height:300px;object-fit:contain;background:#f8fafc;padding:16px;' : 'max-height:320px;object-fit:cover;' }}">
        </div>
        @endif

        <span class="pd-category-tag">
            <i class="fas {{ $publikasi->kategori === 'Buku' ? 'fa-book' : ($publikasi->kategori === 'Artikel' ? 'fa-file-alt' : 'fa-newspaper') }}"></i>
            {{ $publikasi->kategori }}
        </span>
        ...
        <div class="pd-meta">
            ...
            <div class="pd-meta-item"><i class="fas fa-user-edit"></i> {{ $publikasi->penulis }}</div>
            @endif
            @if($publikasi->tanggal_terbit)
            <div class="pd-meta-item"><i class="fas fa-calendar-alt"></i> {{ $publikasi->tanggal_terbit->translatedFormat('d F Y') }}</div>
            @endif
            @if($publikasi->issn)
            <div class="pd-meta-item"><i class="fas fa-barcode"></i> {{ $publikasi->issn }}</div>
            @endif
        </div>
        ...
        <div class="pd-content">
            {!! nl2br(e($publikasi->konten ?? $publikasi->deskripsi)) !!}
        </div>
    ...
    <div class="pd-sidebar">
        <div class="pd-sidebar-card">
            ...
            <div class="pd-sidebar-body">
                ...
                <a href="{{ Storage::url($publikasi->file_url) }}" download class="pd-download-btn">
                    <i class="fas fa-download"></i> Unduh Dokumen
                </a>
                @endif
                <a href="{{ route('publikasi', ['kategori' => $publikasi->kategori]) }}" class="pd-back-btn">
                    <i class="fas fa-arrow-left"></i> Kembali ke {{ $publikasi->kategori }}
                </a>
            </div>
        </div>
        ...
        <div class="pd-sidebar-card">
            <div style="padding:16px 20px;border-bottom:1px solid #f1f5f9;background:#f8fafc;">
                <h4 style="font-size:0.85rem;font-weight:800;color:#0d2b5e;margin:0;display:flex;align-items:center;gap:8px;">
            ...
            <div style="padding:16px 20px;">
                <ul class="pd-info-list">
                    <li>
                        ...
                        <div><span class="pd-info-label">Kategori</span><span class="pd-info-val">{{ $publikasi->kategori }}</span></div>
                    </li>
                    @if($publikasi->penulis)
                    <li>
                        <i class="fas fa-user-edit"></i>
                        <div><span class="pd-info-label">Penulis / Tim</span><span class="pd-info-val">{{ $publikasi->penulis }}</span></div>
                    </li>
                    @endif
                    @if($publikasi->tanggal_terbit)
                    <li>
                        <i class="fas fa-calendar-alt"></i>
                        <div><span class="pd-info-label">Tanggal Terbit</span><span class="pd-info-val">{{ $publikasi->tanggal_terbit->translatedFormat('d F Y') }}</span></div>
                    </li>
                    @endif
                    @if($publikasi->issn)
                    <li>
                        <i class="fas fa-barcode"></i>
                        <div><span class="pd-info-label">ISSN</span><span class="pd-info-val">{{ $publikasi->issn }}</span></div>
                    </li>
                    @endif
                    @if($publikasi->file_url)
                    <li>
                        <i class="fas fa-file-pdf"></i>
                        <div><span class="pd-info-label">File</span><span class="pd-info-val">Tersedia untuk diunduh</span></div>
                    </li>
                    @endif
                </ul>
            </div>
    ...
    <div class="pd-related-grid">
        ...
        <a href="{{ route('publikasi.show', $r) }}" class="pd-related-card">
            ...
            <div class="pd-related-card-body">
                ...
                @if($r->tanggal_terbit)
                <div style="font-size:0.75rem;color:#94a3b8;margin-top:6px;"><i class="fas fa-calendar-alt"></i> {{ $r->tanggal_terbit->format('d M Y') }}</div>
                @endif
            </div>
        </a>
        @endforeach
